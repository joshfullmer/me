...
<div class="flex h-full flex-col items-center">
	...
	<section class="my-auto grid h-full max-h-[420px] w-full max-w-[350px] grid-cols-5 gap-1.5 p-2">
		{#each guesses as guess}
			{#each guess.letters as letter}
				<div
					class="flex aspect-square items-center justify-center text-4xl {getLetterClasses(letter)}"
				>
					{letter.value}
				</div>
			{/each}
